@php
    $heads = [
        'Equipe',
        'Membro',
        ['label' => 'Gerente', 'no-export' => true, 'class' => 'text-center'],
    ...
    $config = \App\Helpers\DataTablesHelper::getConfig([
        'order' => [[0, 'asc'],[1, 'asc']], // Ordenar por equipe (coluna 0) e membro (coluna 1) crescente
        'columnDefs' => [
            ['orderable' => false, 'targets' => [5]] // Desabilitar ordenação nas colunas de ações e botões
        ]
    ]);
@endphp
<x-adminlte.tool.datatable id="membros-equipe-table" :heads="$heads" :config="$config" head-theme="dark" striped bordered
    hoverable>
    @if($projeto->membros()->count())
        @foreach($projeto->membros()->orderBy('equipe_id')->orderBy('created_at', 'desc')->get() as $membro)
            <tr data-equipe="{{ $membro->equipe->descricao }}">
                <td>{{ $membro->equipe->descricao }}</td>
                <td>{{ $membro->nome }}</td>
                ...
                <td>
                    <div class="d-flex justify-content-around w-100">
                        <x-invent::button-dynamic-modal
                            ...
                            icon="fa-solid fa-pencil" label="" theme="" class="rounded-circle text-primary mr-2" title="Editar {{ $membro->nome }}" />
                        ...
                        <x-invent::button-confirm-modal title="Confirmar Remoção"
                            ...
                            class="text-danger rounded-circle" title="Remover {{ $membro->nome }}" />
                    </div>
                </td>
            </tr>
        @endforeach
    @else
        <tr>
            <td colspan="6" class="text-center">{{ __('Nenhum membro encontrado.') }}</td>
        </tr>
    @endif
